...
def find_path_and_setup():
    ...
    assert osp.isfile(osp.join(cutlass_include, 'cutlass/cutlass.h')), cutlass_home
    src_dir = osp.join(current_file_dir, 'src')
    src_files = [osp.join(src_dir, name) for name in
                 ('bindings.cpp', 'gemm.cu', 'element_wise.cu')]

    if sys.platform == 'win32':
        cxx_args = ['/O2', '/std:c++17', '/Zc:__cplusplus']
        nvcc_args = [
        ...
        cxx_args = ['-O3', '-std=c++17', '-fno-strict-aliasing']
